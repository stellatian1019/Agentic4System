...
class FusionPass:
    ...
    @staticmethod
    def _pre_fused_records(graph: Graph) -> list[dict[str, Any]]:
        """Report target fusions already materialized by ONNX export.

        Gemm's optional third input is a fused bias. Official ResNet models
        fold BatchNorm into Conv weights before export, leaving a biased Conv
        and no BatchNormalization nodes. These records intentionally claim no
        node or launch reduction; they only make the existing optimization
        visible through the scoring API requested by the specification.
        """
        records: list[dict[str, Any]] = []
        has_batch_norm = any(
            node.op_type in {"BatchNormalization", "BatchNorm"}
            for node in graph.nodes
        )

        for node in graph.topological_nodes():
            if node.op_type in {"Gemm", "Linear"} and len(node.inputs) >= 3:
                records.append(
                    {
                        "pattern": "FusedMatMulBias",
                        "fused_node": node.name,
                        "original_nodes": [node.name],
                        "inputs": list(node.inputs),
                        "outputs": list(node.outputs),
                        "launches_before": 1,
                        "launches_after": 1,
                        "internal_buffers_removed": 0,
                        "pre_fused": True,
                    }
                )
            elif (
                node.op_type in {"Conv", "Conv2d"}
                and len(node.inputs) >= 3
                and not has_batch_norm
            ):
                records.append(
                    {
                        "pattern": "FusedConv2dBatchNorm",
    ...
    def run(self, graph: Graph) -> dict[str, Any]:
        ...
        fusion_log = self._pre_fused_records(graph)
        ...
        return {
            "changed": any(
                not record.get("pre_fused", False)
                for record in fusion_log
            ),
            "stats": {
